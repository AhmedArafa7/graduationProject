...
@Component({
  ...
  imports: [RouterOutlet, HeaderComponent, FooterComponent, ChatbotComponent], // إضافة هنا
  template: `
    <div class="flex flex-col min-h-screen bg-background-light dark:bg-background-dark font-display text-text-primary dark:text-white" dir="rtl">
      ...
      <app-footer />
      @defer (on idle) { <app-chatbot /> } @placeholder { <div class="fixed bottom-6 left-6 w-16 h-16"></div> }
    </div>
  `
})
